...
import { buildChatPrompt } from '@/lib/ai-training/prompts';
...
function errorResponse(error: unknown) {
  const message = error instanceof Error ? error.message : 'AI 情景训练对话生成失败';
...
export async function POST(request: Request) {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: buildChatPrompt(scenario, validation.data.messages),
    });
